...
const clamp = (px, min, max) => Math.min(max, Math.max(min, Math.round(px)));
export function createLayoutStore() {
    return defineStore({
        ...
        actions: {
            ...
            openDetails: (draft) => {
                if (draft.mobile)
                    draft.sheetOpen = true;
                else
                    draft.details = draft.details === 0 ? DETAILS_DEFAULT : draft.details;
            },
            closeDetails: (draft) => {
...
export function computeColumns(viewport, sidebar, details) {
    if (viewport < MOBILE_BREAKPOINT)
        return { sidebar: 0, center: viewport, details: 0 };
...
export function AppFrame({ useStore, actions, renderSlot }) {
    ...
    const [viewport, setViewport] = useState(() => (typeof window === 'undefined' ? 1024 : window.innerWidth));
    useEffect(() => {
        const measure = () => {
    ...
    return h('div', { className: 'dsh-mobile-frame', 'data-mobile': mobile ? 'true' : 'false', ref: frameRef }, mobile && h('div', { className: 'dsh-mobile-topbar' }, h('button', { type: 'button', onClick: () => actions.toggleSidebar(), 'aria-label': 'menu' }, '☰'), h('button', { type: 'button', onClick: () => actions.openDetails(), 'aria-label': 'details' }, '⋯')), mobile && drawerOpen && h('div', { className: 'dsh-mobile-scrim', onClick: () => actions.toggleSidebar() }), h('aside', {
        className: mobile ? 'dsh-mobile-drawer' : 'dsh-mobile-sidebar',
        'data-open': drawerOpen ? 'true' : 'false',
        style: mobile ? undefined : { width: cols.sidebar },
    }, renderSlot('sidebar', sidebarOwner)), h('main', { className: 'dsh-mobile-center', style: { width: cols.center } }, renderSlot('conversation', {}), renderSlot('shell.overlay', {})), !mobile && h('aside', {
        className: 'dsh-mobile-details',
        'data-open': sheetOpen ? 'true' : 'false',
        style: { width: cols.details },
    }, renderSlot('details', {})), mobile && h('div', { className: 'dsh-mobile-sheet', 'data-open': sheetOpen ? 'true' : 'false' }, h('div', { className: 'dsh-mobile-handle' }), renderSlot('details', {})));
}
// ===== plugin entry =====
