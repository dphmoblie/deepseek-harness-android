...
- **初始设置引导**（P2）：Onboarding 已挂载到首次使用流程（此前为死代码）；引导文本补充受限 root / 工具链 / 权限边界说明。
- **空消息引导**（P2-1）：纯标点/无实质内容输入不再直接发送，提示如何描述任务。
- **错误转译**（P1-3）：errors.ts 新增 EACCES/EPERM/沙箱不可用/apt 不可用等系统限制的中文提示。
