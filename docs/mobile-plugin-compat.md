...
2. dsh-mobile-compat 客户端插件包（AppFrame 移动形态 + aionui-panel 移动 tab）
3. Capacitor 侧 OnboardingSteps（向导五步）
4. rootfs 配方集成点（build-embedded-runtime.py 增加 --mobile-profile 参数，
   默认远程 manifest、按需内嵌）
5. 真机验收清单（荣耀 A16：布局截图矩阵、插件逐项、向导全流程、APK 体积对比）
